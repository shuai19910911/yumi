...
# It selects GPUs with enough free memory and launches supervised SNPWindowFormer.

set -euo pipefail

cd /home/user/zhangzhishuai/myhermes/yumi
mkdir -p results/deep_model/windowformer_supervised_v0_1 logs
...
CUDA_VISIBLE_DEVICES="${GPU_IDS}" mamba run -n yumi python scripts/train_snp_window_transformer_multitask.py \
  ...
  --nhead "${NHEAD}" \
  --amp \
  2>&1 | tee logs/windowformer_supervised_$(date +%Y%m%d_%H%M%S).log
